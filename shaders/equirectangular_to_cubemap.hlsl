...
void main(cs_input IN)
{
	...
	outMip1[texCoord] = srcTexture.SampleLevel(linearRepeatSampler, panoUV, firstMip);

	// Only perform on threads that are a multiple of 2.
	if (numMipLevelsToGenerate > 1 && (IN.groupIndex & 0x11) == 0)
	{
		outMip2[uint3(texCoord.xy / 2, texCoord.z)] = srcTexture.SampleLevel(linearRepeatSampler, panoUV, firstMip + 1);
	}

	// Only perform on threads that are a multiple of 4.
	if (numMipLevelsToGenerate > 2 && (IN.groupIndex & 0x33) == 0)
	{
		outMip3[uint3(texCoord.xy / 4, texCoord.z)] = srcTexture.SampleLevel(linearRepeatSampler, panoUV, firstMip + 2);
	}

	// Only perform on threads that are a multiple of 8.
	if (numMipLevelsToGenerate > 3 && (IN.groupIndex & 0x77) == 0)
	{
		outMip4[uint3(texCoord.xy / 8, texCoord.z)] = srcTexture.SampleLevel(linearRepeatSampler, panoUV, firstMip + 3);
	}
	...
	{
		outMip5[uint3(texCoord.xy / 16, texCoord.z)] = srcTexture.SampleLevel(linearRepeatSampler, panoUV, firstMip + 4);
	}
}
