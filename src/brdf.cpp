...
void dx_cubemap_to_sh_pso::initialize(ComPtr<ID3D12Device2> device)
{
	CD3DX12_DESCRIPTOR_RANGE1 srcMip(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE);
	CD3DX12_DESCRIPTOR_RANGE1 outSH(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE);

	CD3DX12_ROOT_PARAMETER1 rootParameters[cubemap_to_sh_num_params];
	rootParameters[cubemap_to_sh_param_constant_buffer].InitAsConstants(2, 0);
	rootParameters[cubemap_to_sh_param_src].InitAsDescriptorTable(1, &srcMip);
	rootParameters[cubemap_to_sh_param_out].InitAsDescriptorTable(1, &outSH);
	...
	ComPtr<ID3DBlob> shaderBlob;
	checkResult(D3DReadFileToBlob(L"shaders/bin/cubemap_to_sh.cso", &shaderBlob));
